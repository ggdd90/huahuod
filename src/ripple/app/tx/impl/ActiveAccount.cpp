...
ActiveAccount::doApply ()
{
    ...
    {
        bool isHDUTransaction = isHDU (saDstAmount);
        if (mPriorBalance < (isHDUTransaction ? 0 : saDstAmount.xrp ()) + mmm ||
            (isHDUTransaction && sle->getFieldAmount (sfBalanceHDU) < saDstAmount))
        {
        ...
        else
        {
            ...
            {
                view ().peek (keylet::account (account_))->setFieldAmount (sfBalanceHDU,
                    sle->getFieldAmount (sfBalanceHDU) - saDstAmount);
                sleDst->setFieldAmount (sfBalanceHDU,
                    sleDst->getFieldAmount (sfBalanceHDU) + saDstAmount);
            }
